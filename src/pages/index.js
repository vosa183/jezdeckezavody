...
export default function Home() {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      if (evts) {
        ...
        // ŽLUTÁ VYSÍLAČKA - Pípnutí a vibrace při nové zprávě (pouze pro štáb)
  ...
  // ÚPRAVA DRAW PŘÍMO PRO ROZHODČÍHO/SUPERADMINA
  ...
  const saveScore = async () => {
    // KONTROLA: Zápis skóre je povolen pouze Zapisovateli nebo Superadminovi
    const currentRole = profile?.role || simulatedRole;
    if (currentRole !== 'judge' && currentRole !== 'superadmin') {
      ...
      {!user ? (
        ...
        <div style={styles.mainGrid}>
          ...
          <div className="print-area" style={styles.card}>
            ...
            {/* ŽLUTÁ VYSÍLAČKA - PRO ŠTÁB */}
            {['admin', 'superadmin', 'judge', 'speaker'].includes(effectiveRole) && (
              <div className="no-print" style={{ backgroundColor: '#ffff00', border: '4px solid #ccaa00', padding: '15px', marginBottom: '20px', borderRadius: '8px' }}>
                <h3 style={{marginTop: 0, color: '#333'}}>📳 ŽLUTÁ VYSÍLAČKA (TAJNÝ KANÁL ŠTÁBU)</h3>
                <p style={{fontSize: '1.2rem', fontWeight: 'bold', color: '#000', background: '#fff', padding: '10px', border: '1px inset #ccc', minHeight: '40px'}}>
                  {lockedEvent?.internal_message || events.find(e => e.id === adminSelectedEvent || e.id === judgeEvent)?.internal_message || "Žádné nové zprávy."}
                </p>
                <button onClick={() => handleUpdateInternalMessage(lockedEvent?.id || adminSelectedEvent || judgeEvent, lockedEvent?.internal_message || events.find(e => e.id === adminSelectedEvent || e.id === judgeEvent)?.internal_message)} style={{...styles.btnSave, background: '#333'}}>
                  Odeslat zprávu štábu
                </button>
              </div>
            )}
                ...
                {playerTab === 'main' && (
                  <div>
                    ...
                    <div style={{background: '#e3f2fd', padding: '15px', borderRadius: '8px', border: '1px solid #0288d1', marginBottom: '15px'}}>
                      ...
                      <label style={{...styles.label, marginTop: 0, color: '#0288d1'}}>Datum narození jezdce:</label>
                      <input type="date" value={riderBirthDate} onChange={e => setRiderBirthDate(e.target.value)} style={{...styles.inputSmall, border: '2px solid #0288d1', margin: '5px 0 15px 0'}} />
