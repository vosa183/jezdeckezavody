...
export default function PortalPece() {
  ...
  const submitLog = async (e, clubId) => {
    ...
    if (error) {
      ...
      alert('Záznam úspěšně uložen do deníku majitele!'); 
      ...
      {!user ? (
          ...
          <div style={{ textAlign: 'center', marginBottom: '20px' }}>
            {inviteToken ? (
              <div style={{ background: '#e0f7fa', padding: '15px', borderRadius: '8px', border: '2px solid #00838f' }}>
                ...
                <p style={{ margin: 0, color: '#333' }}>Založte účet, nebo se přihlaste.</p>
              </div>
            ) : (
              <>
        ...
        <div style={styles.mainContent}>
          <div style={{ marginBottom: '30px' }}>
            ...
            <p style={{ color: '#555', margin: 0 }}>Zde vidíte všechny koně ze stájí, které vás pozvaly.</p>
          </div>
                ...
                <div key={idx} style={styles.clubCard}>
                  <div style={styles.clubHeader}>
                    ...
                    <span style={styles.roleBadge}>
                      {group.role === 'vet' ? 'VE - Veterinář' : group.role === 'farrier' ? 'KO - Kovář' : group.role}
                    </span>
                  </div>
                  
                  {group.horses.length === 0 ? (
                    <p style={{ color: '#888', padding: '15px' }}>V této stáji nejsou koně.</p> 
                  ) : (
                            ...
                            {isFormOpen && (
                              <form onSubmit={(e) => submitLog(e, group.clubId)} style={styles.logForm}>
                                ...
                                <div style={{ marginBottom: '10px' }}>
                                  ...
                                  <textarea 
                                    placeholder="Co se dělalo, případná medikace..." 
                                    value={newLog.notes} 
