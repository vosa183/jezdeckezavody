...
export default function StajoveImperium() {
    ...
    if (isSignUp) {
      ...
      if (error) {
        ...
        if (!targetClubId) {
          ...
          const { data: newClub } = await supabase.from('clubs').insert([{ name: 'Moje Stáj', is_active: true, trial_ends_at: trialEnd.toISOString() }]).select().single();
  ...
  const toggleDiary = (horseId) => { 
    if (expandedDiaryId === horseId) {
      ...
      setNewLog({ date: new Date().toISOString().split('T')[0], type: 'Jízdárna', notes: '', cost: 0, rating: 0 }); 
    } 
  };
          ...
          {daysArray.map((day, idx) => {
            ...
            return (
              <div 
                ...
                style={{ border: isSelected ? '2px solid #5d4037' : '1px solid #eee', background: isSelected ? '#fff3e0' : '#fff', borderRadius: '4px', minHeight: '35px', padding: '2px', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center' }}
              >
  ...
  const renderLicenseStatus = () => {
    ...
    if (licenseEnd && licenseEnd > now) { 
      ...
      daysLeft = Math.ceil((trialEnd - now) / (1000 * 60 * 60 * 24)); statusText = `Zkušební verze (zbývá ${daysLeft} dní)`; statusColor = '#f57f17'; 
    } else { 
          ...
          <div className={`sidebar ${isSidebarOpen ? 'open' : ''}`} style={styles.sideCard}>
            <div style={{ display: 'flex', marginBottom: '15px', borderBottom: '2px solid #eee' }}>
              <button 
                ...
                style={{ ...styles.tabBtn, borderBottom: sidebarTab === 'profil' ? '3px solid #5d4037' : 'none', color: sidebarTab === 'profil' ? '#5d4037' : '#888' }}
              >
                👤 Profil
              </button>
              <button 
                onClick={() => setSidebarTab('tym')} 
                style={{ ...styles.tabBtn, borderBottom: sidebarTab === 'tym' ? '3px solid #5d4037' : 'none', color: sidebarTab === 'tym' ? '#5d4037' : '#888' }}
              >
                👥 Tým
              </button>
              {(userRole === 'owner' || userRole === 'collaborator') && (
                <button 
                  ...
                  style={{ ...styles.tabBtn, borderBottom: sidebarTab === 'platby' ? '3px solid #5d4037' : 'none', color: sidebarTab === 'platby' ? '#5d4037' : '#888' }}
                >
                  💰 Platby
                </button>
              )}
              {userRole === 'owner' && (
                <button 
                  onClick={() => setSidebarTab('licence')} 
                  style={{ ...styles.tabBtn, borderBottom: sidebarTab === 'licence' ? '3px solid #5d4037' : 'none', color: sidebarTab === 'licence' ? '#5d4037' : '#888' }}
                >
                  🔑 Licence
                </button>
              )}
            ...
            {sidebarTab === 'platby' && (
              <div>
                ...
                {allPayments.length === 0 ? (
                  <p style={{ fontSize: '0.85rem', color: '#888' }}>Nemáte žádné nevyřízené platby.</p>
                ) : (
                  allPayments.map(p => (
                      ...
                      {p.profiles?.bank_account && (
                        <div style={{ marginTop: '10px', textAlign: 'center', background: '#fff', padding: '10px', borderRadius: '8px' }}>
                          <img 
                            ...
                            alt="QR Platba"
                          />
                          <div style={{ fontSize: '0.75rem', color: '#888', marginTop: '5px' }}>Naskenujte v bance</div>
                        </div>
                      )}
                      
                      <button 
                        onClick={() => markAsPaid(p.id)} 
                        style={{ ...styles.btnSave, background: '#4caf50', marginTop: '15px' }}
                      >
                            ...
                            <form onSubmit={(e) => saveDiaryLog(e, h.id)} style={{ display:'grid', gap:'8px', gridTemplateColumns:'1fr 1fr', marginBottom:'25px', background: '#fff', padding: '15px', borderRadius: '8px', border: '1px solid #e3f2fd' }}>
                              ...
                              <div style={{ gridColumn: 'span 2' }}>
                                ...
                                >
                                  ...
                                  <option value="Veterinář">Veterinář / Zuby / Fyzio</option>
